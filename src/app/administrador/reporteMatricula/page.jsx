...
function ReporteMatricula() {
  ...
  const handleGenerateReport = async () => {
    if (!periodoSeleccionado || !idEstudiante) {
      setNotificacion({
    ...
    try {
      const res = await fetch(
        `/api/reportes/certificadoMatricula/${idEstudiante}/${periodoSeleccionado}/`
      );
  ...
  useEffect(() => {
    if (reporte && reporte.ESTUDIANTE) {
      const generatePDF = () => {
        ...
        headerY += 15;
        doc.text(
          `En Ambato a: ${new Date().toLocaleDateString("es-EC", {
            year: "numeric",
            month: "long",
            day: "numeric",
          })}`,
    ...
    <div className="bg-gray-100 dark:bg-gray-800 flex flex-col gap-4 pt-24 pb-10 min-h-screen w-full px-10">
      ...
      <div className="grid grid-cols-1 gap-2 pb-5">
        ...
        <p className="font-light text-lg text-black dark:text-white">
          Certificado de matricula del estudiante.
        </p>
      </div>
...
export default ReporteMatricula;
