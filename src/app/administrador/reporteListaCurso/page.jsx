...
function ListaCurso() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      const res = await fetch(
        `/api/reportes/listaCurso/${campusSeleccionado}/${periodoSeleccionado}/${especialidadSeleccionada}/${nivelSeleccionado}/${paraleloSeleccionado}`
      );
  ...
  useEffect(() => {
    if (reporte.length > 0) {
      const generatePDF = () => {
        ...
        doc.text("LISTADO DE ESTUDIANTES", centerX, headerY, {
          align: "center",
        });
        ...
        const filas = reporte.map((item) => [
          numero++,
          item.ESTUDIANTE.PERSONA.apellido
            .split(" ")
            .map(
              (word) =>
          ...
          item.ESTUDIANTE.PERSONA.nacionalidad.charAt(0).toUpperCase() +
            item.ESTUDIANTE.PERSONA.nacionalidad.slice(1).toLowerCase(),
          item.ESTUDIANTE.PERSONA.sexo.charAt(0).toUpperCase(),
          item.ESTUDIANTE.nombreTrabajo.charAt(0).toUpperCase() +
            item.ESTUDIANTE.nombreTrabajo.slice(1).toLowerCase(),
          item.ESTUDIANTE.PERSONA.parroquia.CANTON.canton
            .split(" ")
            .map(
              (word) =>
                word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()
            ...
            .join(" "),
          item.ESTUDIANTE.PERSONA.telefono,
        ]);
  ...
  const downloadFiles = () => {
    ...
    if (pdfUrl) {
      ...
      link.href = pdfUrl;
      link.download = "reporte_listado_estudiantes.pdf";
      link.click();
    }
  ...
  const generateCSV = (data) => {
    ...
    const filas = data.map(
      (item) =>
        `${numero++};${item.ESTUDIANTE.PERSONA.apellido} ${
          item.ESTUDIANTE.PERSONA.nombre
        };${
          item.ESTUDIANTE.PERSONA.cedula
        };${item.ESTUDIANTE.PERSONA.nacionalidad.toUpperCase()};${item.ESTUDIANTE.PERSONA.sexo
          .charAt(0)
          .toUpperCase()};${item.ESTUDIANTE.nombreTrabajo.toUpperCase()};${item.ESTUDIANTE.PERSONA.parroquia.CANTON.canton.toUpperCase()};${
          item.ESTUDIANTE.PERSONA.telefono
        }`
    );
    ...
    <div className="bg-gray-100 dark:bg-gray-800 flex flex-col gap-4 pt-24 pb-10 min-h-screen w-full px-10">
      ...
      <div className="grid grid-cols-1 gap-2 pb-5">
        ...
        <p className="font-light text-lg text-black dark:text-white">
          Listado de estudiantes de un curso.
        </p>
      </div>
